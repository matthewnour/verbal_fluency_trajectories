function [Mlog,Q]=community_louvain_lex(W,gamma,M0,B)
...
if ~exist('M0','var') || isempty(M0)
    M0=1:n; %start with each node being its own modeule, as this is an agglomerative algorithm 
elseif numel(M0)~=n
    error('M0 must contain n elements.')
end

% initialise each node to a different community ID
[~,~,Mb] = unique(M0);
M = Mb;
...
while Q-Q0>1e-10
    ...
    end %only end this loop if we do not increase modularity
    ...
    Mlog{iteration} = M;                                % make a note of each iteration's node -> community allocation
