...
%   License: BSD 3 clause
    ...
    properties(Constant)
        CYTOMETER_SPECIFIC_DEFAULTS=false;
    ...
    methods
        ...
        function [progressMax, txt]=getProgessMax(this, dataSize)
            ...
            txt=['Running ' SuhJsonSplitter.GetVersionText...
    ...
    methods(Static)
        ...
        function ok=GetUpdate(branch, announceNoNewVersion, jw)
            ...
            pu=PopUp(['<html>Checking Google Cloud<br>for ' ...
                'EPP updates!</html>'], 'center', 'One moment...',...
                true, [],[], false, [], jw);
            if isempty(branch)
                branch=SuhJsonSplitter.DEFAULT_BRANCH;
            end
            ...
            if ~isnan(serverVersion) && (...
                ...
                if serverVersion==localVersion
                    q=Html.WrapHr(['Versions are the same ...' ...
                        '<br>Re-download from Google Cloud anyway?']);
                else
        ...
        function View(jsonFile, csvFile)
            if nargin<2
                csvFile=[];
                if nargin<1
                    jsonFile=[];
            ...
            function advise
                msg(Html.Wrap(['Upload the file:' Html.FileTree(outFile)]));
            end
        end
    end
