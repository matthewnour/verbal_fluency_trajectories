...
%   License: BSD 3 clause
    ...
    methods(Static)
        ...
        function p=DefineArgs(varargin)
            ...
            addParameter(p,'W', .022, ...
                @(x) Args.IsNumber(x, 'W', .001, .5));
            addParameter(p,'min_relative', .005, ...
                @(x) Args.IsNumber(x, 'min_relative', .001, .2));
            addParameter(p,'sigma', 3, ...
                @(x) Args.IsNumber(x, 'sigma', 1, 7));
            addParameter(p,'balance_power', 1, ...
                @(x) Args.IsNumber(x, 'balance_power', 1, 5));
            addParameter(p,'KLD_exponential_1D', .4, ...
                @(x) Args.IsNumber(x, 'KLD_exponential_1D', 0, 1));
            addParameter(p,'KLD_normal_1D', .04, ...
                @(x) Args.IsNumber(x, 'KLD_normal_1D', 0, 1));
            addParameter(p,'KLD_normal_2D', .12, ...
