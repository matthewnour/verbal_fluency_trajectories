...
if isempty(fjt.searchAc) || ~ishandle(fjt.searchAc.fig)
	...
	searchFieldTip=Html.WrapHr('Enter gate names in workspace');
    drawnow;
...
%MatBasics.RunLater(@(h,e)focusSearchField(), .45);
    ...
    function focusPickBtn
        if Gui.IsVisible(fig)
            app.showToolTip(done, doneTip, 75, -45, 0, [], true);
            edu.stanford.facs.swing.Basics.Shake(done,3);
            MatBasics.RunLater(@(h,e)focusPickBtn(), 11.3);
        else
            disp('Done');
        end
    end
...
%                disp(['User entered: "' char(key) '" ids=' ids]);
        ...
        if N==0
            ...
            set(next, 'enabled', false);            
            if op==0                
                msgBox('<html>No matching items found...</html>');
            end
        end
        Gui.HideBusy(fig);
    end
    ...
    function l=findItems(txt)
        ...
        try
            ...
            elseif startsWith(txt,'^')
                l=fjw.search(txt(2:end), true);
            else
                l=fjw.search(txt, true);
            end
        catch
            l=emptyList;
        end
