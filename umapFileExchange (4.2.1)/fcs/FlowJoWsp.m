...
%   License: BSD 3 clause
    ...
    properties(SetAccess=private)
        ...
        sampleKeywords={};
        xmlVersion='version';
        xmlFlowJoVersion='flowJoVersion';
        xmlSample='Sample';
        version;
        flowJoVersion;
        isFlowJo;
    ...
    methods
        ...
        function this=FlowJoWsp(uri)
            this.uri=uri;
            this.staleCountIds=java.util.HashSet;
            located=WebDownload.LocateUri(uri);
            if ~isempty(located)
            ...
            this.version=char(this.doc.item(0).getAttribute(this.xmlVersion));
            this.propsGui=JavaProperties(propsGuiFile);
            this.version=char(this.doc.item(0).getAttribute(this.xmlVersion));
            this.flowJoVersion=char(this.doc.item(0).getAttribute(this.xmlFlowJoVersion));
            this.isFlowJo=~isempty(this.flowJoVersion);
            this.refreshSampleNodes;
            this.xmlEllipseFoci={this.nameFoci, this.nameVertex, ...
                this.nameCoord};
            this.xmlEllipseEdge={this.nameEdge, this.nameVertex, ...
                this.nameCoord};
            this.idMap=Map;
        ...
        function sampleId=getSampleIdByNum(this, sampleNum)
            sampleId=[FlowJoWsp.TYPE_SAMPLE ':' ...
        ...
        function path=getParentIds(this, node, sampleToo, folderToo)
            ...
            if hasPid %already stored in props??
                ...
                if found && sampleToo && folderToo
                    ...
                    while ~isempty(pid) && ~strcmp(FlowJoWsp.ROOT_ID, pid)
                        ids{end+1}=pid;
                    end
                end
                N=length(ids);
            ...
            function found=getId(node)
                ...
                if found
                    if sampleToo
                        ids{end+1}=[FlowJoWsp.TYPE_SAMPLE ':' ...
    ...
    methods
        ...
        function pid=getParentId(this, id)
         ...
         function [r, str]=getExpandableIds(this, ids)
        ...
        function [ok, openBackupFolder, cancelled]=askToSave(this)
            ...
            if this.unsavedChanges>0
                try
                    ...
                    cb=Gui.CheckBox( ...
                        ...
                        true, BasicMap.Global, 'FlowJoWsp.Backup', ...
                        [], Html.Wrap(tip));
            ...
            function shake
                edu.stanford.facs.swing.Basics.Shake(cb, 5);
    ...
    methods
        ...
        function cloned=cloneGraph(this, populationNode, dims)
            names={this.xmlGraph};
            cloned=FlowJoWsp.GetAll(populationNode, names, 1);
            yes=~isempty(cloned);
            if yes
                cloned=cloned{1};
                names{end+1}=this.xmlAxis;
                ax=FlowJoWsp.GetAll(populationNode, names, 1);
                if isempty(ax)
        ...
        function setRoiPosition(this, gate, gateType, newPos, scalers)
            ...
            elseif strcmp(gateType, this.xmlEllipse)
                fociNodes=FlowJoWsp.GetAllWild(gate, ...
                    this.xmlEllipseFoci, 1);
                edgeNodes=FlowJoWsp.GetAllWild(gate, ...
                    this.xmlEllipseEdge, 1);
            ...
            elseif strcmp(gateType, this.xmlEllipse)
                ...
                if ml.length>6
                    ...
                    pos=finisher.finishFlowJoEllipse(...
                        FlowJoWsp.AttributeVector(...
                        FlowJoWsp.GetAllWild(gate, ...
                        this.xmlEllipseEdge, 1), this.nameValue));
                    if this.app.oldRoi
                        this.warn('ellipsoid', ...
