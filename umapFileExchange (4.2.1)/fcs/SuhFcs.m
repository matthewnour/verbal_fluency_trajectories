...
%   License: BSD 3 clause
    ...
    methods
            ...
            if isempty(this.autoGateColumns)
                ...
                for i=1:this.hdr.numDataParameters
                    ...
                    elseif strcmpi('SSC-A', this.hdr.fullColNames{i}) ...
                            || strcmpi('FSC-A', this.hdr.fullColNames{i}) 
                        cols(end+1)=i;
                    end
                end
        ...
        function this=SuhFcs(uri, downloadsFolder, fromWspFile)
            if nargin<3
                fromWspFile=false;
                if nargin<2
                    downloadsFolder=[];
            ...
            this.uri=uri;
            file=WebDownload.LocateUri(uri, downloadsFolder, fromWspFile);
            if ~isempty(file)
        ...
        function data=getDerivedData(this, name)
            data=[];
            ...
            if idx>0
                if isempty(this.derivedParameters.data{idx})
                    pu=PopUp(Html.WrapSmallBold(['<font bgcolor="#FDFDEC">'...
                        'Reading derived paramters</font>']), 'south west');
                    try
                        N=length(this.derivedParameters.names);
                        csvFile=this.derivedParameters.files{idx};
                        [D, ~]=File.ReadCsv2(csvFile);
                        for i=1:N
