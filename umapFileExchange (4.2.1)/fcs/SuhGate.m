...
%   License: BSD 3 clause
    ...
    methods
        ...
        function fn=getFileName(this, idToo)
            if nargin<2
                idToo=true;
            end
            if idToo
                fn=String.ToFile([this.name '_' this.id ]);
            else
                fn=String.ToFile(this.name);
            end
        end
        ...
        function fcn=getEppGateCreatorFunction(this, columns, ask)
            if nargin<3
                ask=true;
            end
            C=length(columns);
            ...
            function pos=store1(key, parent, name, split, count, dims, scalers)
                ...
                [newId, newPop]=this.gml.createSubGate(parent.population, ...
                    parent.id, RoiUtil.POLYGON, pos, name, count, ...
                    dims, scalers);
        ...
        function enableSave(this, askToo, word)
            ...
            function okToClose=answer(~, finalAnswer)
        ...
        function [newPopulation, newGateId]...
            ...
            if asNephew
                ...
                [newPopulation, newGateId, pid]=...
                    nephewGate.clonePopulation('', true);
            end
            this.gater.refreshChildren(pid);
        end
        ...
        function transferPickColumnsToMatch(this)
            this.gml.propsGui.set(FlowJoTree.PROPS_MATCH, ...
                this.gml.propsGui.get(this.getPickColumnsProperty));
        end
    ...
    methods(Static)
        ...
        function [clones, map]=SaveRoiUnderClonedGates(roi, ...
                ...
                cloneIsNephew, args, clones, scalers, map, key)
            isScaled=true;
            if nargin<11
                key=[];
                if nargin<10
