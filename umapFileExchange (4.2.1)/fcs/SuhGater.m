...
%   License: BSD 3 clause
    ...
    methods
        ...
        function this=SuhGater(fcs, gml, displayLimit)
            ...
            this.highlightedGateIds=java.util.TreeMap;
        end
        ...
        function refreshChildren(this, pid, showChild)
            if ~isempty(this.gml) 
                this.gml.resyncChildren(pid);
                if ~isempty(this.tree) && ~isempty(this.tree.suhTree)
                    this.tree.suhTree.ensureChildUiNodesExist(pid);
                    if nargin>2
    ...
    methods
        function yes=setHighlighted(this, gate)
            if isempty(gate)
                ...
                idx=this.highlightedGateIds.get(gate.id);
                if ~isempty(idx)
                    yes=false;
                    this.highlightedGates(idx)=[];
                    this.highlightedGateIds.remove(gate.id); %>5 chars ... no java.lang.String needed
                else
                    this.highlightedGates{end+1}=gate;
                    idx=this.highlightedGateIds.size+1;
                    this.highlightedGateIds.put(gate.id, idx);%>5 chars ... no java.lang.String needed
                    yes=true;
                end
        ...
        function yes=isHighlighted(this, gate)
            yes=this.highlightedGateIds.containsKey(gate.id);
        end
        ...
        function [data, columnNames, labelPropsFile, csvFile,...
            ...
            if askUser(1) && length(columnNames) ~= 1
                [columns, columnNames]=this.fcs.askForColumns(...
                    ...
                    gate.getPickColumnsProperty, BasicMap.Global);
            end
            props=[];
            if isempty(columns)
            ...
            try                
                ...
                if ~justData
                    ...
                    csvFile=fullfile(fldr, ...
                        String.ToFile([gate.name '.' gate.id '.csv']));
