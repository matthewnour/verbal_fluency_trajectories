...
%   License: BSD 3 clause
    ...
    methods(Static)
        ...
        function [names, clrs]=NamesAndColors(lbls, idMap)
            u=unique(lbls)';
            N=length(u);
            names={};
            clrs=[];
    ...
    methods
        ...
        function [classIds, cancelled]=choose(this, make1Column, ...
            ...
            function check
                newRatio=MatBasics.GreaterThan(classIds, 1);
                if newRatio>0
                    ...
                    if ~strcmp(strRatio, strNewRatio)
                        txtMsg=sprintf(['<html><center>For <b>%s</b>'...
                            '<br>the overlap ratio is <b>%s</b>, and '...
                            'the old ratio <br>was <b>%s</b> '...
                            '...is <font color="red"><b>%s</b></font> '...
                            '<u>acceptable</u> ???<hr></center></html>'], ...
                            this.cointextHtml, strNewRatio, ...
                            strRatio, strNewRatio);
                    else
                        txtMsg=sprintf(['<html><center>For <b>%s</b>'...
                            '<br>there is  <b>%s</b> overlap...<br>Is '...
                            'this <u>acceptable</u> ???<hr>'...
        ...
        function this=LabelBasics(contextProperty, ...
                contextDescription, nEvents, props, propsGui, ...
                dataSingular, dataPlural)
            if nargin<7
                dataPlural='data';
                if nargin<6
                    dataSingular='datum';
                    if nargin<5
                        propsGui=JavaProperties;
                        if nargin<4
        ...
        function nEvents=addClass(this, id, ...
            ...
            this.propsGui.set(java.lang.String(id), name);
            key=this.ColorKey(id);
            if nargin>5
                this.propsGui.set(key, color256);
            else
