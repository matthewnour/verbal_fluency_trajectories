...
%   License: BSD 3 clause
    ...
    methods(Static)
        ...
        function c=RobustConcordance(tData, sData, tIdPerRow,tIdSet, sIdPerRow, ...
                sIdSet, areEqual, outlierFraction, verbose)
            c=-1;
            if isempty(tData)||isempty(sData)
                return;
            end
            ...
            try
                ...
                good=sum(tChoices)==length(tOutliers);
                if good
                    tChoices(tChoices) = ~tOutliers;
                else
                    warning('%d tChoices BUT robustcov created %d outliers???', ...
                        sum(tChoices),length(tOutliers));                    
                end
                sChoices=MatBasics.LookForIds(sIdPerRow, sIdSet);
                [~,~,~,sOutliers] = robustcov(sData(sChoices,:),'OutlierFraction',outlierFraction);
                good=sum(sChoices)==length(sOutliers);
                if good
                    sChoices(sChoices) = ~sOutliers;
                else
                    warning('%d sChoices BUT robustcov created %d outliers???', ...
                        sum(sChoices),length(sOutliers));
                end
                sizeRecalled=sum(tChoices&sChoices);
                sizeTrainingSet=sum(tChoices);
                sizeTestSet=sum(sChoices);
