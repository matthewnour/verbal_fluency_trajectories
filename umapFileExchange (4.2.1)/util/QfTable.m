...
%   License: BSD 3 clause
    ...
    methods
        function this=QfTable(qf, tClrs, props, priorFig, ...
            ...
            if ~isForPrediction
                try
                    ...
                    if ~isempty(this.qf.falsePosNegs)
                        ...
                        ToolBarMethods.addButton(tb_, ...
                            fullfile(path, 'phenogram.png'), ...
                            'Visualize by Phenograms and/or MDS', ...
                            @(h,e)seeMore(this, h));
                    end
                catch ex
                    if ~isstruct(this.qf)
            ...
            function heatMapCallback
                this.showHeatMap(nameIdx, idIdx, freqIdx, symIdx, rankIdx);
            end
        ...
        function showHeatMap(this, nameIdx, idIdx, freqIdx, symIdx, rankIdx)
            ...
            [dataIdxs, names, freqs, syms, mdns, compMdns]=...
                ...
                nameIdx, freqIdx, symIdx, idIdx, rankIdx, true); 
        ...
        function ok=doHistF(this, visible)
            ...
            if ishandle(this.fHistFig)
                ...
                if this.qf.areEqual
                    ...
                    this.addTitleToHist(fig_, ax, ...
                        'Overlap ^{(F-measure on bins)}', avgMdn, avgMn);
                end
                set(ax, 'units', 'normalized', 'Position', ...
                    [.1 .16 .8 .7]);
                Gui.StretchUpper(ax, @ylim, .1);
    ...
    methods(Static)
        ...
        function QF=Load(file, complain, tData, tIdPerRow, sIdPerRow)
            QF=[];
            try
                if exist(file, 'file')
                    load(file, 'QF');
        ...
        function [data, names, widths, tips, unmatched, plotIdx, freqIdx,...
                ...
                robustConcIdx, idMap]=Contents(qf, clrs, pu)            
            numIdx=1;
            nameIdx=2;
            matchesIdx=3;
            similarityIdx=4;
            ...
            if hasConcordance %includes concordance
                ...
                idTip=tips{end};
                tips{end}='Percentage of overlap between the two subsets';
                idIdx=13;
                concIdx=11;
                robustConcIdx=12;
                widths(end+1,:)=[4 -1];
                names{end+1}=Html.WrapSm('Robust<br>concordance');
                tips{end+1}='Percentage of overlap between non-outliers in the two subsets';
                widths(end+1, :)=idW;
                names{end+1}=idN;
                tips{end+1}=idTip;
            end
