...
%   License: BSD 3 clause
    ...
    methods(Static)
        ...
        function  file=LocateUri(uri, downloadsFolder, fromWspFile)
            %Example
            %SuhFcs.Locate('https://storage.googleapis.com/cytogenie.org/GetDown2/domains/FACS/demo/bCellMacrophageDiscovery/all_3-3.fcs')
            ...
            if download
                [host,path]=WebDownload.UrlParts(String.URLDecode(uri));
            ...
            elseif decodeFileUrl
                needFlowJoFixing=false;
                if fromWspFile
                    BAD='&+;~=!@$()';
                    N=length(BAD);
                    needFlowJoFixing=any(ismember(uri, BAD))...
                        || contains(uri, '&amp;');
                    if needFlowJoFixing
                        for i=1:N
                            bad=BAD(i);
                ...
                [~,path]=WebDownload.UrlParts(String.URLDecode(uri));
                if needFlowJoFixing
                    idxs=cell(1,N);
                    for i=1:N
                        bad=BAD(i);
                        idxs{i}=strfind(path, bad);
                    end
                end
                path=String.URLDecode(path);
                if needFlowJoFixing
